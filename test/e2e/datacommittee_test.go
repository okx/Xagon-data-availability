...
func startDACMember(t *testing.T, m member) {
	dacNodeConfig := config.Config{
		...
		DB: db.Config{
			...
			Host:      "xlayer-validium-data-node-db-" + strconv.Itoa(m.i),
			Port:      "5432",
			EnableLog: false,
			MaxConns:  10,
		},
...
func stopDACMember(t *testing.T, m member) {
	...
	assert.NoError(t, err, string(out))
	out, err = exec.Command(
		"docker", "kill", "xlayer-validium-data-node-db-"+strconv.Itoa(m.i),
	).CombinedOutput()
	assert.NoError(t, err, string(out))
	out, err = exec.Command(
		"docker", "rm", "xlayer-validium-data-node-db-"+strconv.Itoa(m.i),
	).CombinedOutput()
	assert.NoError(t, err, string(out))
}
